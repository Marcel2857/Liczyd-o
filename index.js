...
function updateIncomes() {
  ...
  const total = incomes.reduce((a, b) => a + Number(b.amount), 0);
...
saveButton.addEventListener("click", () => {
  ...
  incomeToEdit.amount = amountInput.value
  updateIncomes()
})
...
function removeIncome(income) {
  const indexToDelete = incomes.findIndex(
    (item) => item.id === income.id
  );
...
function updateRemainingBudget() {
  ...
  const remainingBudgetValue = incomeTotal - expenseTotal;
